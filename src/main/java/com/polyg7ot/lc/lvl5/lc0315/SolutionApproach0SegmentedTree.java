 ...
 */
package com.polyg7ot.lc.lvl5.lc0315;
...
public class SolutionApproach0SegmentedTree {
    ...
    private void add(int num, SegTreeNode node){
        if(num < node.min || num > node.max) return;
    ...
    private int find(int num, SegTreeNode node){
        ...
        if(num >= node.max) return node.count;
        else{
            int mid = node.getMid();
            if(num < mid) return find(num, node.left);
            else return find(num, node.left) + find(num, node.right);
        }
    }
