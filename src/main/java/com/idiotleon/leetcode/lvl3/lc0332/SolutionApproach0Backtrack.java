...
public class SolutionApproach0Backtrack {
    public List<String> findItinerary(List<List<String>> tickets) {
        ...
        ans.add(START);
        if (findItinerary(START, ans, graph, N + 1))
            return ans;
        return null;
    }

    private boolean findItinerary(String source, List<String> route, Map<String, List<String>> graph, final int LEN) {
        if (route.size() == LEN)
            return true;
        ...
        for (int i = 0; i < destinations.size(); i++) {
            ...
            if (findItinerary(destination, route, graph, LEN))
                return true;
