...
public class SolutionApproach0Backtrack {
    ...
    private void backtrack(char[] chs, StringBuilder abbr, int idx, int digit, List<String> res) {
        final int LEN = abbr.length();
        if (idx == chs.length) {
            /**
             * since type StringBuilder is passed by reference, proper backtrack should be
             * taken to resume to the previous state.
             * 
             * what's more, the "digit" can be larger than 9, such as 10, which consists of
             * 2 digits
             * 
             * to create a new StringBuilder based on "abbr",
             * <code> new StringBuilder(abbr) </code> also works, with much more time&space
             * cost.
             */
            if (digit != 0)
                abbr.append(digit);
