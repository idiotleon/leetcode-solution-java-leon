...
public class SolutionApproach0BinarySearchTree {
    ...
    public boolean book(int start, int end) {
        if (!canBeInserted(start, end, root))
            return false;
    ...
    private boolean canBeInserted(int start, int end, BSTNode node) {
        ...
        if (node == null)
            return true;
        if (start >= node.end)
            return canBeInserted(start, end, node.right);
        else if (end <= node.start)
            return canBeInserted(start, end, node.left);
        else {
            if (node.isOverlapped)
                return false;
            else {
                if (start >= node.start && end <= node.end)
                    return true;
                else
                    return canBeInserted(start, node.start, node.left) && canBeInserted(node.end, end, node.right);
            }
        }
