 ...
 */
package com.idiotleon.leetcode.lvl5.lc0499;
...
public class SolutionApproach0DFS {
    ...
    private void backtrack(int[][] maze, int[] hole, boolean[][] visited, int row, int col, String route, int steps) {
        if (steps > minSteps)
            return;
        ...
        for (int d = 0; d < 4; d++) {
            int r = row, c = col;
            while (r + DIRS[d][0] >= 0 && r + DIRS[d][0] < M && c + DIRS[d][1] >= 0 && c + DIRS[d][1] < N
                    && maze[r + DIRS[d][0]][c + DIRS[d][1]] != 1) {
                r += DIRS[d][0];
                c += DIRS[d][1];
                if (r == hole[0] && c == hole[1] || visited[r][c])
                    break;
            }
