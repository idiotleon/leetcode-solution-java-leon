...
public class SolutionApproach1DFS {
    ...
    private void buildGraph(final String[] words, final boolean[][] adj, final int[] visited) {
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            if (i > 0) {
                final String w1 = words[i - 1], w2 = words[i];
                ...
                for (int j = 0; j < len; j++) {
                    final char c1 = w1.charAt(j), c2 = w2.charAt(j);
                    if (c1 != c2) {
                        adj[c1 - 'a'][c2 - 'a'] = true;
                        break;
                    }
