 ...
 */
package com.an7one.leetcode.lvl3.lc0401;
...
public class SolutionApproach0Backtrack {
    public List<String> readBinaryWatch(int num) {
        ...
        for (int pick = 0; pick <= Math.min(4, num); pick++) {
            ...
            if (num - pick > 6)
                continue;
            List<Integer> minutesCombinations = new ArrayList<Integer>(), hoursCombinations = new ArrayList<Integer>();
    ...
    private void backtrack(final int[] CANDIDATES, final int AT_MOST_PICKS, int picked, int curTime, int start,
            List<Integer> res, final int LIMIT) {
