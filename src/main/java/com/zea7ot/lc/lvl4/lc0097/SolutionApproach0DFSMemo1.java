...
public class SolutionApproach0DFSMemo1 {
    ...
    private boolean isInterleave(int idx1, int idx2, int idx3,
        ...
        if(isInvalid[idx1][idx2]) return false;
        if(idx3 == L3) return true;
        
        boolean isValid = (idx1 < L1 && chs1[idx1] == chs3[idx3] && isInterleave(idx1 + 1, idx2, idx3 + 1, chs1, chs2, chs3, isInvalid)) 
            || (idx2 < L2 && chs2[idx2] == chs3[idx3] && isInterleave(idx1, idx2 + 1, idx3 + 1, chs1, chs2, chs3, isInvalid));
        if(!isValid) isInvalid[idx1][idx2] = true;
        return isValid;
    }
}