...
public class SolutionApproach0DP1Dimen2 {
    public int coinChange(int[] coins, int amount) {
        ...
        // the actual value of int[] dp means the minimum steps to reach that certain
        // amount/index
        ...
        for (int i = 0; i <= amount; i++) {
            if (dp[i] == -1)
                continue; // this means this amount/index cannot be constructed by any chance
            for (int j = 0; j < L; j++) {
                // this means the current construct is too large than then amount/index targeted
                // at,
